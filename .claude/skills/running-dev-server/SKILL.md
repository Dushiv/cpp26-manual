...
- The engine loads lesson content via `fetch("../content/modules/<locale>/mN.json")`, relative to `prototype/index.html` — so the server root must be the **repo root**, not `prototype/`. Serving from `prototype/` makes these requests 404 and the app shows "Не удалось загрузить контент урока...".
...
# or
curl -sI http://127.0.0.1:8901/prototype/index.html | head -1
```
...
Run this from the **repo root** with a background launch (e.g. `run_in_background`). Then open `http://127.0.0.1:8901/prototype/index.html`.
...
- **Serving from `prototype/` instead of the repo root** — `loadCourseData()`'s `fetch("../content/modules/...")` then resolves outside the served root and 404s, producing the "Не удалось загрузить контент урока..." error screen.
